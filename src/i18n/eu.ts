export default {
  // App
  'app.url': 'https://demo.sidebadger.com/dashboard',
  'api.baseUrl': 'https://demo.sidebadger.com',

  // Meta
  'meta.title': 'Side Badger - Fakturak banatzeko, gastuak partekatzeko eta side-badger-ak',
  'meta.description': 'Side Badger guztiz doakoa da, jarraipen eta iragarkirik gabeko faktura banatzailea, lagunekin gastuak banatzea errazten duena. Sortu taldeak, gehitu gastuak eta konturatu erraz. Dramarik gabe, faktura banaketa sinplea.',

  // Header
  'header.logoText': 'Side Badger',
  'header.launchApp': 'Aplikazioa Ireki',

  // Footer
  'footer.tagline': 'Guztiz doakoa. Jarraipen gabe. Iragarki gabe.<br />Azkonarrak fakturak banatzen.',
  'footer.product': 'Produktua',
  'footer.pricing': 'Prezioak',
  'footer.launchApp': 'Aplikazioa Ireki',
  'footer.legal': 'Legala',
  'footer.privacy': 'Pribatutasun Politika',
  'footer.terms': 'Zerbitzu Baldintzak',
  'footer.cookies': 'Cookie Politika',
  'footer.copyright': 'Side Badger. Eskubide guztiak erreserbatuak (baina ez benetan, doakoa da).',
  'footer.madeWith': 'Kaosarekin eta kafearekin egina',

  // Hero
  'hero.badge': '% 100 doakoa eta mugagabea',
  'hero.disclaimer': '* Jakina, muga batzuk daude. Erabilera zuzena eta beste muga asko aplikatzen dira. Doakoa da ez den arte.',
  'hero.title': 'Banatu gastuak',
  'hero.titleHighlight': 'azkonarra bezala',
  'hero.titleDisclaimer': '',
  'hero.subtitle': 'Guztiz doakoa, jarraipenik gabeko, iragarkirik gabeko gastu banatzailea, susmagarriki ona dena. Ez dugu zure daturik nahi. Ez dugu zure dirurik nahi. Zure lagunekin fakturak banatu nahi dugu negar egin gabe.',
  'hero.cta': '...azkonarratu nazazu',

  // Features section
  'features.title': 'Behar Duzun Guztia',
  'features.subtitle': 'Ezaugarri guztiak. Jarraipen gabe. Doktoregoa behar gabe.',

  // Feature cards
  'features.groups.title': 'Taldeak Guztirako',
  'features.groups.desc': 'Gelakideak, bidaiak, ekitaldiak, D&D festak. Sortu behar dituzun talde guztiak eta mantendu gastuak antolatuta.',

  'features.invites.title': 'Gonbidapen Errazak',
  'features.invites.desc': 'Partekatu esteka bat, lagunak elkartzen dira. Aplikaziorik deskargatu beharrik gabe. Aukeran, onartu behar da kide berriek sartu aurretik.',

  'features.roles.title': 'Baimen Konfiguragarriak',
  'features.roles.desc': 'Kontrolatu nork gehitu ditzakeen gastuak, nork kontura dezakeen, edo nork ikus dezakeen aldetik. Zure taldea, zure arauak.',

  'features.splits.title': 'Hiru Banatzeko Modu',
  'features.splits.desc': 'Berdina (berdin banatu), Zehatza (zenbateko zehatzak), edo Ehunekoa. Aukeratu zure taldeari egokitzen zaiona. Nori ez zaio gustatzen 3-ko bat?',

  'features.participants.title': 'Aukeratu Nor Dagoen',
  'features.participants.desc': 'Ez da dena guztien artean banatzen. Aukeratu zehaztasunez nor dagoen gastu bakoitzean parte hartzen. "Nik entsalada bat bakarrik jan nuen" orain aukera bat da.',

  'features.currency.title': 'Edozein Moneta',
  'features.currency.desc': 'USD, EUR, JPY, GBP, edozein. Moneta guztien laguntza osoa dezimalen tratamendu egokiarekin. Azken erabilitakoen aukeraketa azkarra.',

  'features.balances.title': 'Saldo Sinplifikatuak',
  'features.balances.desc': 'Zor sinplifikazio adimentsua beharrezko ordainketa kopurua minimizatzen du. Guztiek guztiei ordaindu beharrean, konpontzeko bide laburrena aurkitzen dugu.',

  'features.settle.title': 'Klik Bakarreko Konponketa',
  'features.settle.desc': 'Zor bat ikusten duzu? Sakatu "Konponketa Azkarra" eta markatu ordainduta bezala. Eginda. Konfetia aukerakoa da baina oso gomendatua.',

  'features.activity.title': 'Jarduera Jarioa',
  'features.activity.desc': 'Ikusi gertatzen den guztia: gastu berriak, konponketak, kideak elkartzen. Mantendu zure taldearekin sinkronizatuta "itxaron, zer gertatu da?" galdetu gabe.',

  'features.comments.title': 'Txateatu eta Iruzkindu',
  'features.comments.desc': 'Talde txata eztabaida orokorrerako, gehi iruzkinak banakako gastuetan. "Nork eskatu zituen nacho gehigarriak?" Mantendu elkarrizketa dagokion tokian.',

  'features.edit.title': 'Konpondu Akatsak',
  'features.edit.desc': 'Zenbateko okerra sartu duzu? Editatu gastuak sortu ondoren. Aldatu banaketa, konpondu data, eguneratu deskribapenak. Estresik gabe.',

  'features.noTracking.title': 'Jarraipen Gabe',
  'features.noTracking.desc': 'Analitikarik gabe. Jarraipen pixelik gabe. Datu bilketarik gabe. Ez dakigu zertan gastatzen duzun dirua, eta horrela gustatzen zaigu.',

  // Coming soon section
  'comingSoon.title': 'Laster Etorriko Da',
  'comingSoon.subtitle': 'Hasi berria gara. Hona hemen zer datorren.',

  'comingSoon.conversion.title': 'Moneta Bihurketak',
  'comingSoon.conversion.desc': 'Moneten arteko bihurketa automatikoa. Banatu eurotan, konpondu dolarretan.',

  'comingSoon.receipts.title': 'Tiketak Igotzea',
  'comingSoon.receipts.desc': 'Erantsi tiketen argazkiak edozein gasturi. Inoiz ez galdu gastatutakoaren jarraipena.',

  'comingSoon.ai.title': 'AA Tiket Eskaneatzea',
  'comingSoon.ai.desc': 'Atera argazki bat, utzi AAri xehetasunak ateratzen. Gastu sorrera azkarragoa idazketa gutxiagorekin.',

  'comingSoon.export.title': 'Esportatu Zure Datuak',
  'comingSoon.export.desc': 'Deskargatu zure gastuak Excel, CSV edo PDF formatuan. Zure datuak, zure formatua, zure modura.',

  'comingSoon.mobile.title': 'Mugikorrerako Aplikazio Natiboak',
  'comingSoon.mobile.desc': 'iOS eta Android aplikazioak bidean gastuen jarraipena egiteko. Gehitu gastuak ordaindu bezain laster.',

  // Little things section
  'littleThings.title': 'Garrantzia Duten Gauza Txikiak',
  'littleThings.subtitle': 'Gastuak banatzea atsegin bihurtzen duten xehetasunak.',

  'littleThings.dates.title': 'Data Azkarrak',
  'littleThings.dates.desc': '"Gaur", "Atzo" eta "Bart" botoiak. Inork ez duelako data hautatzailea erabili nahi afari baterako.',

  'littleThings.amounts.title': 'Azken Zenbatekoak',
  'littleThings.amounts.desc': 'Maiz erabiltzen dituzun zenbatekoak agertzen dira aukeraketa azkarrerako. Alokairua beti berdina da? Ukitu bat.',

  'littleThings.names.title': 'Izan Nahi Duzuna',
  'littleThings.names.desc': 'Talde bakoitzeko pantaila izen desberdina. "Ama" familian, "Miren" lanean, "Festazalea" lagunekin.',

  'littleThings.archive.title': 'Artxibatu Talde Zaharrak',
  'littleThings.archive.desc': 'Bidaia bukatu da? Artxibatu. Taldea seguru geratzen da baina zure bidetik kanpo. Itzuli edozein momentutan.',

  'littleThings.celebrations.title': 'Ospakizunak',
  'littleThings.celebrations.desc': 'Guztiak konponduta daudenean, konfetia jasotzen duzu. Gauza txikiak dira.',

  'littleThings.everywhere.title': 'Nonahi Funtzionatzen Du',
  'littleThings.everywhere.desc': 'Mahaigaina, tableta, telefonoa. Deskargatzeko aplikaziorik gabe. Zure nabigatzailean funtzionatzen du.',

  // Why free section
  'whyFree.title': 'Zergatik da doakoa?',
  'whyFree.subtitle': 'Fakturak banatzeak ez duelako dirua kostatu behar. Hori zentzugabea da.',

  'whyFree.trackers.title': 'Zero Jarraipen',
  'whyFree.trackers.desc': 'Analitikarik gabe. Jarraipen pixelik gabe. "Bazkideekin parteka dezakegu" gabe. Zure gastu datuak zureak izaten jarraitzen dute.',

  'whyFree.ads.title': 'Iragarkirik Gabe',
  'whyFree.ads.desc': '47,23 €-ko afari faktura bat banatzea nahikoa gogaikarria da janari-kit harpidetzetarako iragarkirik behar gabe.',

  'whyFree.badgers.title': 'Azkonarrak Bakarrik',
  'whyFree.badgers.desc': 'Side Badger doako gastu-banaketa aplikazioa da, pribatutasuna lehenesten duena. Ezkutuko agendarik gabe. Bibrazio eta azkonarrak bakarrik.',

  // CTA section
  'cta.title': 'Faktura batzuk banatzeko prest?',
  'cta.subtitle': 'Batu azkonarra iraultzara. Doakoa da, pribatua da, eta pixka bat kaotikoa da modu onenean.',
  'cta.button': 'Ireki Side Badger',

  // Pricing page
  'pricing.meta.title': 'Prezioak - Side Badger | Orain Doakoa',
  'pricing.meta.description': 'Side Badger une honetan doakoa da ezkutuko kosturik, iragarkirik eta jarraipen gabe. Jakin zergatik eskaintzen dugun doako.',
  'pricing.hero.price1': '99€/hilabete',
  'pricing.hero.price2': '49€/hilabete',
  'pricing.hero.price3': '9€/hilabete',
  'pricing.hero.finalPrice': '0€',
  'pricing.hero.subtitle': 'Orain doakoa. Kreditu txartelik behar gabe.',

  // Full Badger Plan
  'pricing.plan.badge': 'Uneko Plana',
  'pricing.plan.name': 'Azkonarra Osoa',
  'pricing.plan.currency': '€',
  'pricing.plan.amount': '0',
  'pricing.plan.period': '/hilabete',
  'pricing.plan.description': 'Guztia. Dena. Mugarik gabe irauten duen bitartean.',
  'pricing.plan.feature1': 'Talde mugagabeak',
  'pricing.plan.feature2': 'Gastu mugagabeak',
  'pricing.plan.feature3': 'Kide mugagabeak taldeko',
  'pricing.plan.feature4': 'Zero jarraipen',
  'pricing.plan.feature5': 'Zero iragarki',
  'pricing.plan.feature6': 'Azkonarra energia osoa',
  'pricing.plan.feature7': 'Barnetik datorren epeltasun sentsazioa',
  'pricing.plan.cta': 'Hasi Fakturak Banatzen',

  // Enterprise Plan (joke)
  'pricing.enterprise.badge': 'Enpresa',
  'pricing.enterprise.name': 'Mega Azkonarra',
  'pricing.enterprise.amount': '???',
  'pricing.enterprise.period': '/???',
  'pricing.enterprise.description': 'Bilera bat izateko bilera bat behar duzunerako.',
  'pricing.enterprise.feature1': 'Azkonarra Osoko guztia',
  'pricing.enterprise.feature2': 'PDF txukun bat',
  'pricing.enterprise.feature3': 'Sinergia optimizazioa',
  'pricing.enterprise.feature4': 'Blockchain zerbait',
  'pricing.enterprise.feature5': 'AA-k elikatutako buzzword-ak',
  'pricing.enterprise.feature6': 'Hitz egiteko pertsona erreal bat*',
  'pricing.enterprise.cta': 'Jarri Salmenten Harremanetan',
  'pricing.enterprise.note': '*Azkonarrei buruz bakarrik hitz egingo dugu',
  'pricing.enterprise.alert': '🦡 HONK HONK! Txantxetan, ez dago enpresa mailarik. Doako planak dena du. Zoaz hori erabili!',

  // FAQ Section
  'pricing.faq.title': 'Izan Ditzakezun Galderak',
  'pricing.faq.q1.question': 'Benetan doakoa da?',
  'pricing.faq.q1.answer': 'Bai, une honetan guztiz doakoa da. Ez ditugu ezaugarriak ordainketa-horma baten atzean ezkutatzen. Doan mantentzen dugu, edonork estresik gabe gastuak banatu ahal izan ditzan, guk gauza berriak argitaratzen ditugun bitartean.',
  'pricing.faq.q2.question': 'Zein da tranpa?',
  'pricing.faq.q2.answer': 'Ez dago tranparik. Side Badger existitzen da gastu-banaketa erraza, pribatua eta dibertigarria izan dadin. Hori da istorio osoa.',
  'pricing.faq.q3.question': 'Nire datuak salduko dituzu?',
  'pricing.faq.q3.answer': 'Inola ere ez. Ez dugu aplikazioa martxan jartzeko beharrezkoa dena baino datu pertsonalik biltzen. Zero jarraipen, zero analitika pixel eta zero interes zure eroste ohituretan.',
  'pricing.faq.q4.question': 'Zergatik azkonarra?',
  'pricing.faq.q4.answer': 'Zergatik EZ azkonarra? Azkonarrak erabakitsuak, baliabidez hornituak eta taldeetan ondo lan egiten dute. Zu bezala Airbnb horretarako nork zer zor duen jakiten saiatzen zarenean.',
  'pricing.faq.q5.question': 'Nire negozioan erabil dezaket?',
  'pricing.faq.q5.answer': 'Jakina! Zure negozio txiki edo talderako partekatutako gastuak jarraitu nahi badituzu, aurrera. Doakoa da. Ez dugu epaitzen.',
  'pricing.faq.q6.question': 'Zer gertatzen da akatsa aurkitzen badut?',
  'pricing.faq.q6.answer': 'Lehenik eta behin, nola ausartzen zara gure kodeak akatsak dituela iradokitzen? Bigarrenik, bai, ziurrenik akatsak daude. Azkar bidaltzen ari gara, beraz izan goxo gurekin.',

  // Policy pages
  'policy.privacy.title': 'Pribatutasun Politika',
  'policy.privacy.meta.title': 'Pribatutasun Politika - Side Badger',
  'policy.privacy.meta.description': 'Gure pribatutasun politikak Side Badger-ek zure datuak nola kudeatzen dituen azaltzen du. Spoiler-a: ez zaitu jarraitzen, ez ditu zure datuak saltzen, ez ditu iragarkirik erakusten.',
  'policy.terms.title': 'Zerbitzu Baldintzak',
  'policy.terms.meta.title': 'Zerbitzu Baldintzak - Side Badger',
  'policy.terms.meta.description': 'Gure zerbitzu baldintzek Side Badger nola erabil dezakezun azaltzen dute. TL;DR: izan lasai, ez egin legez kanpoko gauzarik eta gozatu doako bidaiaz.',
  'policy.cookies.title': 'Cookie Politika',
  'policy.cookies.meta.title': 'Cookie Politika - Side Badger',
  'policy.cookies.meta.description': 'Gure cookie politikak Side Badger-ek zer cookie erabiltzen dituen azaltzen du. Spoiler-a: ez ditugu jarraipen cookie-ak erabiltzen, aplikazioa funtzionatzeko oinarrizkoak bakarrik.',

  // Language switcher
  'languageSwitcher.search': 'Bilatu...',
  'languageSwitcher.noResults': 'Ez da hizkuntzarik aurkitu',

  // 404 page
  'notFound.meta.title': 'Orria Ez Da Aurkitu - Side Badger',
  'notFound.meta.description': 'Ups! Orri hau ez da existitzen. Azkonarra galdu egin da.',
  'notFound.title': 'Orria Ez Da Aurkitu',
  'notFound.description': 'Badirudi azkonarra bidetik kanpo atera dela. Bilatzen ari zaren orria ez da existitzen.',
  'notFound.backHome': 'Itzuli Hasierara',

  // Accessibility
  'a11y.skipToContent': 'Saltatu eduki nagusira',
  'a11y.changeLanguage': 'Aldatu hizkuntza',
  'a11y.searchLanguages': 'Bilatu hizkuntzak',
  'a11y.mainNavigation': 'Nabigazio nagusia',
  'a11y.footerNavigation': 'Oinarako nabigazioa',
  'a11y.openInNewTab': 'Fitxa berri batean irekitzen da',
  'a11y.externalLink': 'Kanpo esteka',
  'a11y.currentPage': 'Uneko orria',
  'a11y.loading': 'Kargatzen',
  'a11y.homeLink': 'Joan hasiera orrira',

  // Coming Soon badge
  'comingSoon.badge': 'Laster Etorriko Da',

  // Policy page
  'policy.loading': 'Politika kargatzen...',
  'policy.error.generic': 'Ezin izan da politika kargatu.',
  'policy.error.config': 'Konfigurazio errorea.',
  'policy.error.status': 'Ezin izan da politika kargatu ({status} errorea).',
  'policy.error.unavailable': 'Politika ez dago eskuragarri.',
  'policy.error.empty': 'Politikaren edukia hutsik dago.',
  'policy.error.tryAgain': 'Mesedez, saiatu berriro geroago edo jarri laguntza zerbitzuarekin harremanetan.',
} as const;
